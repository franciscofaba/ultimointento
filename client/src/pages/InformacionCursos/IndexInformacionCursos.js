...
function InformacionCursos() {
    ...
    useEffect(() => {
        ...
        const fetchTable = async () => {
            try {
                const answer = await axios.get(`http://127.0.0.1:5000/api/CourseInProgress/${idUser_fk}`);
                ...
                if (firstIdCareer) {
                    const gradocarreraResponse = await axios.get(`http://127.0.0.1:5000/api/informaciongrados/${firstIdCareer}`);
    ...
    const fetchInformacion = async (idUser_fk) => {
        try {
            const response = await axios.get(`http://127.0.0.1:5000/api/informacioncursos/${idUser_fk}`);
